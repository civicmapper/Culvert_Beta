...
def return_periods(capacity_table, current_runoff_table, future_runoff_table, return_periods_output_table, final_output_filename):
    """
    Given culvert capacity and peak discharge from storm events, determine the
    highest return period storm that a culvert can pass for current and future
    rainfall conditions.
    """
    ...
    current_runoff_data = loader.load(current_runoff_table, runoff_signature, 1, -1)
    current_runoff_rows = current_runoff_data['valid_rows']

    future_runoff_data = loader.load(future_runoff_table, runoff_signature, 1, -1)
    ...
    culvert_data = loader.load(capacity_table, culvert_signature, 1, -1)
    ...
    for culvert in culvert_rows:
        ...
        try:
            ...
            click.echo("Did not find watershed for barrierID {0}".format(culvert['BarrierID']))
    ...
    with open(return_periods_output_table, 'wb') as return_output_file:
    ...
    with open(final_output_filename, 'wb') as final_output_file:
        ...
        for culvert in culvert_rows:
            watershed = culvert['watershed']
            if watershed == None:
                click.echo("Skipping culvert {0}".format(culvert['BarrierID']))
                continue # Skip all the culverts we couldn't match with watersheds.
            csv_writer.writerow([
                culvert['BarrierID'],
                culvert['NAACC_ID'],
