...
def geometry(field_data_input_filename, output_filename):
    ...
    Input:
        - culvert_field_data.csv with the following columns: BarrierID,
        Field_ID, Lat, Long,Rd_Name, Culv_Mat, In_Type, In_Shape, In_A, In_B,
        HW, Slope, Length, Out_Shape, Out_A, Out_B, Comments, Flags
    
    Outputs:
        - culvert_capcity_input: a new csv file that contains all necessary
    ...
    field_data_signature = [
        ...
        #{'name': 'Out_A', 'type': float}, #Tanvi Naidu (6/16/2017)
        {'name':'HW','type':float}, ##changed from 'Out_A' to 'HW'
        {'name': 'Flags', 'type': int}
    ];
