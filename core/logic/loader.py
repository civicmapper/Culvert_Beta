    ...
    try:
        with open(filename, 'r') as csv_file:
            ...
            if len(missing_headers) > 0:
                click.echo("ERROR: file '{0}' was missing the following required headers on row {1}:\n\t{2}\nBailing Out.".format(filename, str(start_row), ", ".join(missing_headers)))
                sys.exit(0)
                return None
            ...
            for row in input_table:
                ...
                for header in required_headers:
                    ...
                    try:
                        ...
                        "row_number": row_number,
                        "row": row,
                        "reason_invalid": "in row {0}, column {1} ({2}) of file {3}, the value {4} could not be parsed to {5}.".format(
                            str(row_number),
                            column_string(index + 1),
                            header['name'],
                            filename,
        ...
        click.echo( "ERROR: Could not find file '{0}'. Bailing Out".format(filename))
        sys.exit(0)
        return None
