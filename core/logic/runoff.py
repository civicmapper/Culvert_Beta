...
def calculate(sorted_watershed_table, watershed_precipitation_table, rainfall_adjustment, output_filename):
    """
    Determine the runoff peak flow using the SCS curve number method (see TR-55
    document for further details)
    
    Inputs:
        - sorted_watershed_table: culvertID, watershed area (sq km), average
        curve number, time of concentration (hr)
        - watershed_precipitation_table: csv file exported from the Cornell NRCC of 24 hour storms
        with return periods 1 to 500 years
        - rainfall_adjustment: scalar, with 1 as current rainfall.
        - output_filename: where to save the results of the runoff calculation.
    
    Outputs:
        - table of runoff (q_peak) in cubic meters per second for each return periods under current precipitation conditions
        - table of runoff (q_peak) in cubic meters per second for each return periods under future precipitation conditions
        
    Precipitation values (cm) are average for the overall watershed, for 24 hour
    storm, from NRCC 1yr,2yr,5yr,10yr,25 yr,50 yr,100yr,200 yr,500 yr storm
    Rainfall values are read directly from the standard NRCC output format and
    converted into cm.
    """
    ...
    watershed_data = loader.load(sorted_watershed_table, watershed_data_signature, 1, -1)
    #Header in row 1, and we want to read all rows (max rows= -1)
    valid_watersheds = watershed_data['valid_rows']

    # Load precipitation data.
    precip_data = loader.load(watershed_precipitation_table, precip_data_signature, 10, 9)
    ...
    if len(precip_rows) < 9:
        click.echo("ERROR: failed to load all precipitation data from file '{0}'. Bailing out.".format(watershed_precipitation_table))
        sys.exit(0)
    ...
    if len(skipped_watersheds) > 0:
        ...
        click.echo("NOTE: skipped {0} watersheds.".format(
